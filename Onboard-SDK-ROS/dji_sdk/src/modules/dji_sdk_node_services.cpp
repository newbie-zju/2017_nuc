...
{
	//ROS_ERROR_THROTTLE(0.3,"dji_attitude_control: %d, %4.2f,%4.2f,%4.2f",(uint8_t)request.flag,request.x,request.y,request.z);
...
bool DJISDKNode::local_position_control_callback(dji_sdk::LocalPositionControl::Request& request, dji_sdk::LocalPositionControl::Response& response)
{
    ...
    */
    ...
    float Kp = 0.7;
    flight_ctrl_data.flag = 0x90;
    flight_ctrl_data.x = Kp * (dst_x - local_position.x);
    flight_ctrl_data.y = Kp * (dst_y - local_position.y);
    flight_ctrl_data.z = dst_z;
    flight_ctrl_data.yaw = request.yaw;
...
bool DJISDKNode::takeOffandLand_control_callback(dji_sdk::LocalPositionControl::Request& request, dji_sdk::LocalPositionControl::Response& response)
{
	float dst_x = request.x;
    float dst_y = request.y;
    float dst_z = request.z;
    ...
    flight_ctrl_data.flag = 0x80;
    
	//flight_ctrl_data.x = dst_x;
	//flight_ctrl_data.y = dst_y;
	
	flight_ctrl_data.x = 0.0;
